...
public class CpnvertString {
    private int findMinOperations(String s1,String s2,int i1,int i2){
        ...
        int insesrtOp = 1 + findMinOperations(s1,s2,i1,i2+1);
        int replaceOp =  1 + findMinOperations(s1,s2,i1 + 1,i2 + 1);

        return Math.min(deleteOp,Math.min(insesrtOp,replaceOp));

    }
 public int stringConvert(String s1,String s2){
